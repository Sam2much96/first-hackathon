...
def approval():
    ...
    local_attack = Bytes("Attack")  # uint64
    local_defense = Bytes("Defense")  # byteslice
    local_hp = Bytes("reveal")  # byteslice
    ...
    def reset(account: Expr):
        return Seq(
            ...
            App.localPut(account, local_attack, Int(0)),
            App.localPut(account, local_defense, Int(0)),
            App.localPut(account, local_commitment, Bytes("")),
            App.box_put(account, Int(0), ) # Should store Tuples to Box Storage
            App.localPut(account, local_reveal, Bytes("")), # Store ASA id
        )
